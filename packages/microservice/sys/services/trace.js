...
module.exports = (ms) => {
  ...
  const resolvers = {
    Query: {
      traceFacets: async () => {
        const operations = await ms.monitor.Trace.aggregate([
          {
            $group: {
              _id: '$operation',
              count: { $sum: 1 }
        ...
        const modules = await ms.monitor.Trace.aggregate([
          {
            $group: {
              _id: '$module',
              count: { $sum: 1 }
        ...
        const dates = await ms.monitor.Trace.aggregate([
          {
            $group: {
              _id: 'date',
              max: { $max: '$date' },
      ...
      traces: (
        ...
        return ms.monitor.Trace.find(trace)
          .sort(sort)
          .skip(offset)
          .limit(size)
          .exec()
