...
module.exports = (ms) => {
  ...
  const vercelDeployV2 = () => {
    const urls = ms.config.get('vercel.deploy.hooks') || []
    urls.forEach((url) => {
  ...
  const checkVercelDeployStatusV2 = async () => {
    const urls = ms.config.get('vercel.deploy.status') || []
      ...
      promises.push(
        new Promise((resolve, reject) => {
          fetch(url, {
            method: 'get',
            headers: {
              Authorization: 'Bearer ' + ms.config.get('vercel.deploy.token')
            }
          })
            .then((res) => res.json())
            .then((data) => {
      ...
      startDeploy: async (_, { _id }) => {
        ...
        if (_id) {
          ...
          if (!existingDeployment)
            return new ApolloError('deployment not found')
        ...
        serviceNames.forEach((serviceName) =>
          promises.push(
            new Promise((resolve, reject) => {
              fetch(
                ms.config.get(`..services.${serviceName}.url`) + '/graphql',
                {
                  method: 'post',
                  body: JSON.stringify({
